...
export const Route = createFileRoute("/security/virtual-employees/$virtualEmployeeId")({
...
function VirtualEmployeeDetailPage() {
  const { virtualEmployeeId } = Route.useParams();
  ...
  const employee = useQuery({ queryKey: workspace.key("virtual-employee", virtualEmployeeId), queryFn: () => api.getVirtualEmployee(virtualEmployeeId) });
  const spend = useQuery({ queryKey: workspace.key("virtual-employee-spend", virtualEmployeeId), queryFn: () => api.getVirtualEmployeeSpend(virtualEmployeeId), enabled: tab === "spend" });
  const audit = useQuery({ queryKey: workspace.key("virtual-employee-audit", virtualEmployeeId), queryFn: () => api.getVirtualEmployeeAudit(virtualEmployeeId), enabled: tab === "audit" });
  const refresh = () => {
    void queryClient.invalidateQueries({ queryKey: workspace.key("virtual-employee", virtualEmployeeId) });
    void queryClient.invalidateQueries({ queryKey: workspace.key("virtual-employees") });
  };
  ...
  return (
    <div className="space-y-6">
      <Link to="/security/virtual-employees" className="inline-flex min-h-11 items-center gap-2 rounded-md text-sm text-muted-foreground hover:text-foreground focus-visible:outline-2"><ArrowLeft className="size-4" /> Virtual Employees</Link>
      <PageHeader
