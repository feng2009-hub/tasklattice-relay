...
export const Route = createFileRoute("/security/virtual-employees/")({
...
function VirtualEmployeesPage() {
  ...
  const action = useMutation({
    ...
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: workspace.key("virtual-employees") }),
  });
            ...
            <tbody className="divide-y">
              {filtered.map((employee) => (
                <tr key={employee.id} className="group hover:bg-muted/20">
                  <td className="px-4 py-4"><Link to="/security/virtual-employees/$virtualEmployeeId" params={{ virtualEmployeeId: employee.id }} className="block min-w-48 rounded-sm focus-visible:outline-2"><strong className="block font-medium group-hover:text-primary">{employee.displayName}</strong><span className="mt-0.5 block font-mono text-[11px] text-muted-foreground">{employee.name}</span></Link></td>
                  ...
                  <td className="px-3 py-3">
                    <DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" aria-label={`Actions for ${employee.displayName}`}><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">
                      <DropdownMenuItem asChild><Link to="/security/virtual-employees/$virtualEmployeeId" params={{ virtualEmployeeId: employee.id }}>View details</Link></DropdownMenuItem>
