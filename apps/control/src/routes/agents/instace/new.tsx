            ...
            <Card>
              ...
              <CardContent className="space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2"><Label>Model Profile</Label>{inferenceGroups.isPending ? <div className="flex min-h-12 items-center border bg-muted/20 px-3 text-sm text-muted-foreground">Checking available Model Profiles…</div> : inferenceGroup ? <div className="flex min-h-12 items-center gap-3 border border-emerald-500/25 bg-emerald-500/5 px-3 py-2"><span className="grid size-8 shrink-0 place-items-center bg-emerald-500/10 text-emerald-700"><RouteIcon className="size-4" /></span><span className="min-w-0"><strong className="block truncate text-sm">{inferenceGroup.name}</strong><span className="block text-xs text-muted-foreground">Ready · {search.inferenceGroupId ? "selected for this Instance" : "platform default"}</span></span></div> : <div className="flex min-h-12 items-center border border-amber-500/30 bg-amber-500/5 px-3 text-xs">{requestedInferenceGroupUnavailable ? "The selected Model Profile is not ready or no longer exists" : "No ready Model Profile is available"}</div>}<p className="text-xs leading-5 text-muted-foreground">The profile supplies the endpoint, model alias, routing policy, compliance boundary, and isolated Virtual Key.</p></div>
                </div>
