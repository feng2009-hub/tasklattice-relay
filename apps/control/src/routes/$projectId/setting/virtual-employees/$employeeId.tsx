...
import { formatPlatformDate, formatPlatformDateTime } from "@/lib/platform-preferences";
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="audit" className="mt-5">{audit.isPending ? <div className="h-64 animate-pulse bg-muted/50" /> : audit.error ? <p role="alert" className="text-destructive">{audit.error.message}</p> : <Card><CardHeader><CardTitle>Audit activity</CardTitle><CardDescription>Provisioning, access, credential, synchronization, and binding events.</CardDescription></CardHeader><CardContent className="divide-y">{audit.data.length ? audit.data.map((event) => <div key={event.id} className="grid gap-1 py-4 sm:grid-cols-[1fr_auto]"><div><strong className="text-sm">{event.type.replaceAll(".", " ")}</strong><p className="mt-1 text-xs text-muted-foreground">{event.message}</p></div><div className="text-xs text-muted-foreground sm:text-right"><span className="block">{event.actor}</span><time>{formatPlatformDateTime(event.createdAt)}</time></div></div>) : <p className="py-5 text-sm text-muted-foreground">No audit activity recorded.</p>}</CardContent></Card>}</TabsContent>
      </Tabs>
