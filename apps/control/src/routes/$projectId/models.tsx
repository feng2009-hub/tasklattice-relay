...
function ProvidersPage() {
  ...
  const costs = useQuery({
    queryKey: scope.key("provider-cost", range),
  ...
  const accountCosts = useQuery({
    queryKey: scope.key("provider-cost-breakdown", range),
...
function AccountActions({ account, models, onAddModel }: { account: ProviderAccount; models: ModelDeployment[]; onAddModel: () => void }) {
  ...
  const scope = useProjectQueryScope();
  const revalidate = useMutation({ mutationFn: () => api.revalidateProviderAccount(account.id), onSuccess: async () => Promise.all([queryClient.invalidateQueries({ queryKey: scope.key("provider-accounts") }), queryClient.invalidateQueries({ queryKey: scope.key("model-deployments") })]) });
  const remove = useMutation({ mutationFn: () => api.deleteProviderAccount(account.id), onSuccess: async () => Promise.all([queryClient.invalidateQueries({ queryKey: scope.key("provider-accounts") }), queryClient.invalidateQueries({ queryKey: scope.key("model-deployments") })]) });
