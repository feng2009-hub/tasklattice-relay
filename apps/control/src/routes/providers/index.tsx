...
function MobileAccountCard({ account, models, onAddModel, spend }: { account: ProviderAccount; models: ModelDeployment[]; onAddModel: () => void; spend?: number | undefined }) {
  ...
  return <article className="p-4"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><strong className="block truncate">{account.name}</strong><span className="mt-2 flex items-center gap-2 text-xs text-muted-foreground"><ProviderIcon presetId={account.presetId} className="size-8 shrink-0 [&_img]:size-5" />{providerPresets.find((item) => item.id === account.providerKind)?.name ?? account.providerKind}</span></div><AccountActions account={account} onAddModel={onAddModel} /></div><div className="mt-4 grid grid-cols-3 gap-3 text-xs"><span><span className="block text-muted-foreground">Models</span><strong>{models.length}</strong></span><span><span className="block text-muted-foreground">Spend</span><strong>{spend === undefined ? "—" : money(spend)}</strong></span><span><span className="block text-muted-foreground">Status</span><StatusBadge status={account.status} /></span></div><Button variant="ghost" className="mt-3 w-full justify-between" onClick={() => setOpen((value) => !value)}>Connection details<ChevronDown className={cn("transition-transform", open && "rotate-180")} /></Button>{open ? <div className="mt-3"><AccountDetails account={account} models={models} /></div> : null}</article>;
}
