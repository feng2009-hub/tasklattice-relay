...
export function ProviderRegistrationDrawer({
  ...
  const create = useMutation({
    ...
    onSuccess: async () => {
      setStep("summary");
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: scope.key("provider-accounts") }),
        queryClient.invalidateQueries({ queryKey: scope.key("model-deployments") }),
  ...
  const retryFailures = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: scope.key("model-deployments") });
    },
  });
...
function ExistingAccountModelForm({ account, onCreated }: { account: ProviderAccount; onCreated: () => void }) {
  ...
  const add = useMutation({ mutationFn: api.registerModelDeployment, onSuccess: async (model) => { await queryClient.invalidateQueries({ queryKey: scope.key("model-deployments") }); if (model.status === "VALIDATED") onCreated(); } });
