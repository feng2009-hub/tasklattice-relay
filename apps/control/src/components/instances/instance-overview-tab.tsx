    ...
    return (
      <Card className="border-destructive/25 bg-destructive/[0.025]">
        ...
        <CardContent className="space-y-4">
          <p role="alert" className="border-l-2 border-destructive bg-destructive/5 px-3 py-3 text-sm text-destructive">
...
function CapabilitySection({ action, children, description, icon: Icon, title }: {
  action: ReactNode;
  children: ReactNode;
  description: string;
  icon: typeof Bot;
...
function NamedCapabilityList({ empty, items }: {
  empty: string;
...
function ContextFact({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="grid min-h-12 grid-cols-[minmax(7rem,.8fr)_minmax(0,1.2fr)] items-center gap-4 border-b py-2 last:border-b-0">
      ...
      <dd className="min-w-0 break-words text-right text-xs font-semibold">{value}</dd>
    </div>
  );
}

function capabilitySentence({ allowedToolCount, knowledgeCount, memoryEnabled, serverCount, skillCount, toolAccessKnown }: {
...
export function InstanceOverviewTab({ access, agent, modelRoutingName, platform }: {
  ...
  const selectedServers = (agent.mcpServerIds ?? []).map((id) =>
    ...
    <div role="tabpanel" aria-label="Overview" className="space-y-5 pt-5">
        ...
        <div className="grid lg:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.75fr)]">
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <h2 id="agent-capability-profile" className="font-display text-2xl font-medium tracking-tight sm:text-3xl">
                What {agent.name} can do
              </h2>
              <InstanceStatusBadge status={agent.status} />
            </div>
            <p className="mt-4 max-w-3xl text-base leading-7">
              <strong>{roleName}.</strong> {roleDescription}
            </p>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {terminalFirst && access.terminal.enabled ? (
                ...
                <Button disabled className="min-h-11">Start a task <ExternalLink /></Button>
              )}
              <Button asChild variant="outline" className="min-h-11">
          ...
          <div className="border-t bg-muted/20 p-6 sm:p-8 lg:border-l lg:border-t-0">
            ...
            <dl className="mt-4">
              <ContextFact label="Role" value={roleName} />
              <ContextFact
        ...
        <Card>
          ...
          <CardContent>
            ...
            <CapabilitySection
              ...
              action={(
                <Button asChild variant="link" className="min-h-11 px-0">
                  <Link {...capabilityHref} hash="skills">Review</Link>
                </Button>
              )}
            >
              {catalog.isPending && selectedSkills.length ? (
                <p className="text-xs text-muted-foreground">Loading assigned skills…</p>
              ) : (
                <NamedCapabilityList
                  items={selectedSkills}
                  empty="No specialist skill packages are attached. The Agent follows its role instructions only."
                />
              )}
            </CapabilitySection>
            ...
            <CapabilitySection
              ...
              action={(
                <Button asChild variant="link" className="min-h-11 px-0">
                  <Link {...capabilityHref} hash="mcp-servers">Review access</Link>
                </Button>
              )}
            >
              {catalog.isPending || policies.isPending ? (
                <p className="text-xs text-muted-foreground">Calculating effective tool access…</p>
              ) : catalog.isError || policies.isError ? (
                <p role="alert" className="text-xs text-destructive">
                  Effective tool access could not be verified. Review Capabilities for details and recovery.
                </p>
              ) : (
                <NamedCapabilityList
                ...
                items={[
                  ...
                  ...(agent.memory
                    ? [{
                        ...
                        description: agent.memory.mode === "hybrid"
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Button asChild variant="outline" className="mt-4 min-h-11 w-full">
                <Link {...capabilityHref}>Inspect effective access <ArrowRight /></Link>
              </Button>
            </CardContent>
          </Card>

          <Card>
            <DetailCardHeader title="Ways to work" description="Choose the surface that matches the task." />
            <CardContent className="space-y-3">
      ...
      <div className="flex flex-col gap-3 border-t py-4 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        <span className="flex items-center gap-2">
          ...
          Capability changes are enforced by the Agent's assigned resources and access policies.
        </span>
        <Button asChild variant="link" className="min-h-11 justify-start px-0 sm:justify-center">
          <Link
            to="/$projectId/instances/$instanceId"
            params={{ projectId, instanceId: agent.id }}
            search={{ tab: "logs" }}
