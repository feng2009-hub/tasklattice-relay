...
import { Bot, ChevronDown, ServerCog, ShieldCheck } from "lucide-react";
      ...
      footer={
        <div className="flex w-full flex-col-reverse gap-2 sm:flex-row sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              variant="outline"
              disabled={mutation.isPending || loading || !canSave}
    ...
    >
      ...
      {!loading ? (
        <div className="space-y-8">
          <section aria-labelledby="permission-details-heading" className="space-y-5">
            <div>
              <h2 id="permission-details-heading" className="font-serif text-xl font-semibold">
                Permission details
              </h2>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Select discovered MCP tools, then choose whether each tool is allowed or denied.
              </p>
            </div>
          ...
          <section aria-labelledby="bindings-heading" className="space-y-4 border-t pt-7">
            <div>
              <h2 id="bindings-heading" className="font-serif text-xl font-semibold">
                Bindings
              </h2>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Apply this policy to Virtual Employees and all of their bound Instances.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
        ...
        <CollapsibleTrigger asChild>
          ...
          >
            <span className="flex min-w-0 items-center gap-3">
              {server.logoUrl ? <img src={server.logoUrl} alt="" className="size-6 shrink-0 object-contain" /> : <ServerCog className="size-5 shrink-0" />}
              <span className="min-w-0">
                <span className="flex flex-wrap items-center gap-2">
                  <strong className="truncate font-serif text-base">{server.name}</strong>
                  <Badge variant="outline">{server.tools.length} tools</Badge>
                </span>
                <span className="mt-1 block text-xs text-muted-foreground">
                  Default {defaultDecision.toLowerCase()} · explicit Tool decisions override
                </span>
              </span>
            </span>
