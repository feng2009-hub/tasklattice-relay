    ...
    >
      ...
      >
        <form onSubmit={(event) => event.preventDefault()} className="min-w-0 space-y-5">
          ...
          {step === 1 ? (
            <Card>
              <CardHeader><CardTitle className="flex items-center gap-2"><Bot className="size-5" /> Runtime & Security</CardTitle><CardDescription>Choose the runtime implementation, Virtual Employee, and independent OpenShell Runtime Policy.</CardDescription></CardHeader>
              <CardContent className="space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <form.Field name="virtualEmployeeId">{(field) => <div className="space-y-2"><div className="flex items-center justify-between gap-3"><Label>Virtual Employee</Label><Link to="/$projectId/setting" params={{ projectId }} search={{ section: "virtual-employees" }} className="text-xs font-medium underline underline-offset-4">Manage Virtual Employees</Link></div><Select value={field.state.value} disabled={virtualEmployees.isPending || Boolean(virtualEmployees.error)} onValueChange={field.handleChange}><SelectTrigger className="min-h-12 h-auto" aria-label="Virtual Employee"><SelectValue placeholder={virtualEmployees.isPending ? "Loading Virtual Employees…" : "Select a Virtual Employee"} /></SelectTrigger><SelectContent>{activeVirtualEmployees.map((employee) => <SelectItem key={employee.id} value={employee.id}>{employee.displayName}</SelectItem>)}</SelectContent></Select>{virtualEmployees.error ? <p role="alert" className="text-xs text-destructive">{virtualEmployees.error.message}</p> : !virtualEmployees.isPending && !activeVirtualEmployees.length ? <p role="alert" className="border-l-2 border-amber-500 bg-amber-500/5 px-3 py-2 text-xs"><Link to="/$projectId/setting" params={{ projectId }} search={{ section: "virtual-employees" }} className="font-semibold underline underline-offset-4">Create and activate a Virtual Employee</Link> to continue.</p> : <p className="text-xs leading-5 text-muted-foreground">Provides the Project permission identity and bound Access Policy.</p>}</div>}</form.Field>
                </div>
                <form.Subscribe selector={(state) => state.values.virtualEmployeeId}>{(virtualEmployeeId) => { const selected = activeVirtualEmployees.find((item) => item.id === virtualEmployeeId); const bindings = (accessPolicies.data ?? []).filter((policy) => policy.virtualEmployeeIds.includes(String(virtualEmployeeId))); return selected ? <div className="grid gap-3 border-y py-4 text-xs sm:grid-cols-2"><div><span className="text-muted-foreground">Permission identity</span><strong className="mt-1 block">{selected.displayName}</strong></div><div><span className="text-muted-foreground">Binding Policy</span><strong className="mt-1 block">{bindings.map((policy) => policy.name).join(", ") || "No Access Policy bound"}</strong></div></div> : null;}}</form.Subscribe>
