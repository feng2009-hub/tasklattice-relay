...
export function CreateInstanceSheet({
  ...
  const shellProps = {
    description: "Define the Agent, choose one virtual Project member, and apply an independent OpenShell Runtime Policy.",
    ...
    >
      ...
      >
        <form onSubmit={(event) => event.preventDefault()} className="min-w-0 space-y-5">
          ...
          {step === 1 ? (
            <Card>
              <CardHeader><CardTitle className="flex items-center gap-2"><Bot className="size-5" /> Runtime & Security</CardTitle><CardDescription>Choose the runtime implementation, virtual Project member, and independent OpenShell Runtime Policy.</CardDescription></CardHeader>
              <CardContent className="space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <form.Field name="virtualEmployeeId">{(field) => <div className="space-y-2"><div className="flex items-center justify-between gap-3"><Label>Virtual member</Label><Link to="/$projectId/setting" params={{ projectId }} search={{ section: "members" }} className="text-xs font-medium underline underline-offset-4">Manage members</Link></div><Select value={field.state.value} disabled={virtualEmployees.isPending || Boolean(virtualEmployees.error)} onValueChange={field.handleChange}><SelectTrigger className="min-h-12 h-auto" aria-label="Virtual member"><SelectValue placeholder={virtualEmployees.isPending ? "Loading virtual members…" : "Select a virtual member"} /></SelectTrigger><SelectContent>{activeVirtualEmployees.map((employee) => <SelectItem key={employee.id} value={employee.id}>{employee.displayName} · {employee.businessRole || employee.environment}</SelectItem>)}</SelectContent></Select>{virtualEmployees.error ? <p role="alert" className="text-xs text-destructive">{virtualEmployees.error.message}</p> : !virtualEmployees.isPending && !activeVirtualEmployees.length ? <p role="alert" className="border-l-2 border-amber-500 bg-amber-500/5 px-3 py-2 text-xs"><Link to="/$projectId/setting" params={{ projectId }} search={{ section: "members" }} className="font-semibold underline underline-offset-4">Create and activate a virtual member</Link> to continue.</p> : <p className="text-xs leading-5 text-muted-foreground">Provides the Project member identity, model access, backing identities, and approved system scope.</p>}</div>}</form.Field>
                </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    <div className="grid gap-5 sm:grid-cols-2">
                      ...
                      <ReviewSection title="Runtime & Security"><ReviewRow label="Runtime" value="OpenShell (UAT)" /><ReviewRow label="Agent" value={getAgentPlatformPresentation(values.agentPlatform).name} /><ReviewRow label="Virtual member" value={activeVirtualEmployees.find((item) => item.id === values.virtualEmployeeId)?.displayName ?? "Unavailable"} /><ReviewRow label="Runtime Policy" value={policyName(values.policyId)} /></ReviewSection>
                    </div>
                    <Separator />
