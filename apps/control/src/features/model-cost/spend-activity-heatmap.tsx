...
const intensityColors = [
  "var(--muted)",
  "color-mix(in srgb, var(--brand-signal) 22%, var(--background))",
  "color-mix(in srgb, var(--brand-signal) 42%, var(--background))",
  "color-mix(in srgb, var(--brand-signal) 66%, var(--background))",
  "var(--brand-signal)",
];
...
export function SpendActivityHeatmap({
  ...
  return (
    <Card className="gap-0 py-0 shadow-none">
      <CardHeader className="flex min-h-14 flex-row items-center justify-between gap-4 border-b px-4 py-3">
        ...
        <TooltipProvider>
          <div className="overflow-x-auto pb-2">
            <div className="min-w-max">
              <div className="mb-2 ml-10 grid h-4 gap-1.5 text-[10px] text-muted-foreground" style={{ gridTemplateColumns: `repeat(${weekCount}, 0.75rem)` }}>
                {monthLabels.map((label, index) => <span key={`${label}-${index}`} className="whitespace-nowrap">{label}</span>)}
              </div>
              <div className="flex gap-2">
                {mode !== "weekly" ? (
                  <div className="grid grid-rows-7 gap-1.5 pr-1 text-[10px] leading-3 text-muted-foreground">
                    {weekdays.map((day, index) => <span key={day}>{index % 2 ? day : ""}</span>)}
                  </div>
                ) : <div className="w-8 text-[10px] text-muted-foreground">Week</div>}
                <div
                  className={cn("grid gap-1.5", mode === "weekly" ? "grid-flow-col grid-rows-1" : "grid-flow-col grid-rows-7")}
                >
                  {gridCells.map((cell) => {
                      ...
                      <Tooltip key={`${mode}-${cell.date}`}>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            className={cn(
                              "size-3 rounded-[2px] border border-black/[0.035] transition-transform hover:scale-125 focus-visible:z-10 focus-visible:scale-125 focus-visible:outline-2 focus-visible:outline-offset-1",
