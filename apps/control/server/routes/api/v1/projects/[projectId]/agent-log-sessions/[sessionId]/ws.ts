...
export default defineWebSocketHandler({
  async upgrade(request) {
    ...
    if (
      ...
      || detail.kind !== "A2A"
      || detail.status !== "READY"
      || !detail.instance.podName
    ) {
      throw new Response("The managed A2A Agent Pod is not available.", { status: 409 });
    }
  ...
  async open(peer) {
    ...
    try {
      ...
      if (!detail || detail.kind !== "A2A") {
        throw new Error("Agent Instance is no longer available.");
      }
      peer.send(
        `[control] Following ${detail.runtimeView.namespace}/${detail.runtimeView.podName} container agent (read-only)\r\n`,
      );
      const logs = new KubernetesManagedAgentLogStream();
      connection.handle = await logs.open(
        session.projectId,
        detail.instance,
        session.options,
        {
